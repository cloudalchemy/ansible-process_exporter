...
| `process_exporter_binary_local_dir` | "" | Allows to use local package instead of the one distributed on github. As parameter it takes a directory where `process_exporter` binary is stored on host on which ansible is ran. This overrides `process_exporter_version` parameter
